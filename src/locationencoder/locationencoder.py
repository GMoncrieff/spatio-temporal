...
# Minimal local loss-fn factory to avoid dependency on utils.losses (not present in this repo)
def get_loss_fn(presence_only=False, loss_weight=None, regression=False):
    """
    Returns a callable loss function with signature loss_fn(module, lonlats, label).
    This keeps the LocationEncoder self-contained for forward feature extraction.

    - For regression=True: uses MSE between model outputs and labels.
    - For classification: uses CrossEntropyLoss on logits.
    """
    if regression:
        mse = nn.MSELoss()
        def _loss_fn(module, lonlats, label):
            preds = module(lonlats)
            return mse(preds.squeeze(), label)
        return _loss_fn
    else:
        ce = nn.CrossEntropyLoss()
        def _loss_fn(module, lonlats, label):
            logits = module(lonlats)
            return ce(logits, label)
        return _loss_fn
...
class LocationEncoder(pl.LightningModule):
    ...
    def test_step(self, batch, batch_idx):
        # Lazy import to avoid OpenMP conflicts at import-time
