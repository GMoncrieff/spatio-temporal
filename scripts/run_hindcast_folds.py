...
hindcast input window, then stitch the per-fold rasters into one global raster per
(window, horizon, quantile).
...
from src.prediction.stitch import (  # noqa: E402
    HORIZONS,
    QUANTILES,
...
# The 512 px coarse-block mask. The 128 px checkerboard (fold_mask_1000.tif) is NOT what any
# shipped checkpoint was trained against; its fold tile is inside the residual correlation
# length, so held-out skill measured on it is optimistic.
FOLD_MASK = HM_DIR / "fold_mask_b4_1000.tif"
...
def parse_args(argv=None):
    ...
    p.add_argument("--output_root", default="data/hindcast")
    p.add_argument("--norm_stats_json", default="data/norm_stats.json")
    p.add_argument("--log_dir", default="data/logs")
...
def build_fold_command(args, fold, windows):
    ...
    if args.disable_wandb:
        ...
        cmd += [
            ...
            "--wandb_tags", f"hindcast,fold-cv,fold{fold}",
        ]
    return cmd
...
def main(argv=None):
    ...
    print(f"\n✓ Stage '{args.stage}' complete in {elapsed:.1f} min")
    if run is not None:
