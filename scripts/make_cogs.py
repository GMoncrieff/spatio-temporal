...
def translate(src: Path, dst: Path, dst_nodata=None) -> None:
    dst.parent.mkdir(parents=True, exist_ok=True)
    override = ["-a_nodata", str(dst_nodata)] if dst_nodata is not None else []
    cmd = ["gdal_translate", "-of", "COG", *CREATION, *override, str(src), str(dst)]
    r = subprocess.run(cmd, capture_output=True, text=True)
    if r.returncode != 0:
        raise SystemExit(f"gdal_translate failed for {src}:\n{r.stderr.strip()[-2000:]}")


def verify(src: Path, dst: Path, n_windows: int = 12, seed: int = 42, dst_nodata=None) -> str:
    ...
    with rasterio.open(src) as a, rasterio.open(dst) as b:
        ...
        # With --dst_nodata the header is being corrected on purpose, so check it against
        # what was asked for rather than against the source it is fixing.
        want = a.nodata if dst_nodata is None else dst_nodata
        if not (want is None and b.nodata is None) and not (
            want == b.nodata
            or (want is not None and b.nodata is not None
                and np.isnan(want) and np.isnan(b.nodata))
        ):
            raise SystemExit(f"{dst}: nodata {b.nodata} != expected {want}")
...
def main(argv=None):
    ...
    ap.add_argument("--dst_nodata", default=None,
                    help="Override the output nodata value, e.g. 'nan'. Use when the source "
                         "raster's header disagrees with what it actually stores -- the "
                         "prediction writer used to inherit 3.4e38 while filling with NaN.")
    ap.add_argument("--overwrite", action="store_true")
    args = ap.parse_args(argv)

    src_dir, out_dir = Path(args.src_dir), Path(args.out_dir)
    dst_nodata = float(args.dst_nodata) if args.dst_nodata is not None else None
    ...
    for year in years:
        for q in QUANTILES:
            ...
            if dst.exists() and not args.overwrite:
                ...
                translate(src, dst, dst_nodata)
            print(f"  {dst.name}  ({dst.stat().st_size / 1e9:.2f} GB)  {verify(src, dst, dst_nodata=dst_nodata)}")
            written.append(dst)
