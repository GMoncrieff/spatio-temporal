...
# Inlined from the ensemble branch's src.ensemble.validate.DIST_BINS. The binning
# convention is itself a measurement -- an exact Euclidean transform makes dist == 1.0
# one of the most populated values on this grid -- so do not re-derive these edges.
DIST_EDGES = [0.0, 1.0, 3.0, 10.0, 30.0, 100.0, np.inf]

REGION_ROOT = Path("data/ensemble/region/global")

# Bands from the T8 table in the plan, so the numbers line up with the ones already
# measured for the quantile heads. They label DIST_EDGES above, which is the single
# definition on this branch; _bin below uses right=True, which is that convention.
